...
源码仓库只提交源码、测试、脚本和配置文件。`dist/`、`bin/`、`obj/`、本地 NuGet 缓存和临时目录都不应该提交。

如果要发布可下载的软件安装包或压缩包，建议使用 GitHub Releases 上传 `dist/` 里生成的 zip 或平台成品。

## 发布前检查

当前图片处理依赖 `Magick.NET-Q8-AnyCPU` 已升级到 `14.13.1`。

正式发布前建议运行下面命令检查依赖安全状态：

```powershell
dotnet list .\ModernImageViewer.slnx package --vulnerable
```
